...
void TablePage::Init(page_id_t page_id, size_t page_size,
  ...
  if (ENABLE_LOGGING) {
    LogRecord logRecord(txn->GetTransactionId(),txn->GetPrevLSN(),LogRecordType::NEWPAGE,prev_page_id);
 ...
 */
    void TablePage::CLRinsert (const RID &rid , Transaction *txn , LogManager *log_manager , lsn_t undo_next) {
  if (ENABLE_LOGGING) {
    // must have already grab the exclusive lock
      Tuple deleted;
      GetTuple(rid,deleted,txn);
      LogRecord logRecord(txn->GetTransactionId(),txn->GetPrevLSN(),LogRecordType::CLR,undo_next);
      lsn_t cur_lsn=log_manager->AppendLogRecord(logRecord);
      txn->SetPrevLSN(cur_lsn);
      SetLSN(cur_lsn);
  }
...
int32_t TablePage::GetFreeSpaceSize() {
  return GetFreeSpacePointer() - 24 - GetTupleCount() * 8;
    ...
    void TablePage::CLRdelete (const RID &rid , Transaction *txn , LogManager *logManager , lsn_t undo_next) {
    ...
    void TablePage::CLRupdate (const RID &rid , Transaction *txn , LogManager *logManager , lsn_t undo_next ,
                               const Tuple &prev) {
        bool tmp=ENABLE_LOGGING;
        Tuple now;
