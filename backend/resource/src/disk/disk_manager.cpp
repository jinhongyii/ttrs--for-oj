...
//        log_io_.close();
        ...
        if (!db_io_.is_open()||GetFileSize(file_name_)==0) {
            ...
            db_io_.seekg(-4,std::ios::end);
            db_io_.read(reinterpret_cast<char*>(&next_page_id_),sizeof(next_page_id_));
        }
    ...
    DiskManager::~DiskManager() {
        ...
        db_io_.write(reinterpret_cast<char*>(&next_page_id_),sizeof(next_page_id_));
        db_io_.close();
 ...
 */
    ...
    void DiskManager::clear () {
        db_io_.close();
        db_io_.open(file_name_,std::fstream::out|std::ios::trunc);
        db_io_.close();
        log_io_.close();
        log_io_.open(log_name_,std::ios::out|std::ios::trunc);
        log_io_.close();
