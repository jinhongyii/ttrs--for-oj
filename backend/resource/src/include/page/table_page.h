...
public:
	...
	void CLRinsert(const RID &rid, Transaction *txn, LogManager *log_manager, lsn_t undo_next); // when commit abort
	void CLRdelete(const RID &rid, Transaction *txn, LogManager *logManager, lsn_t undo_next);

	void CLRupdate(const RID &rid, Transaction *txn, LogManager *logManager, lsn_t undo_next, const Tuple &prev);
