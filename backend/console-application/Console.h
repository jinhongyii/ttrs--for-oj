    ...
    public:
        ...
        bool processline(std::istream& is,std::ostream& os){
            ...
            if (tmp == string<10>("register")) {
                ...
                unsigned long long userid;
                is>>userid;
                user* user1=usermanager.search(userid);
                if (user1 == nullptr) {
                    os<<0<<"\n";
                    return false;
                }
                os<<*user1<<"\n";
                usermanager.bpm->UnpinPage(user1->getpage_id() , false , 1);
                return false;
                ...
                ticketQuery.~TicketQuery();
                ticketManager.~TicketManager();
                new (&usermanager) UserManager(&recordbpm);
