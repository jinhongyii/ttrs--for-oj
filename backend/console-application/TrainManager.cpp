...
sjtu::TrainManager::TrainManager (sjtu::TicketQuery *ticketQuery) : ticketQuery(ticketQuery),
    ...
    HeaderPage* head= reinterpret_cast<HeaderPage*>(recordbpm.FetchPage(0 , 0));
    ...
    if(head->GetRootId("trains",first_page)){
        ...
        head->InsertRecord("trains",trainTable->GetFirstPageId());
    }
    if(head->GetRootId("loctimeprice",first_page)) {
        ...
        head->InsertRecord("loctimeprice",loctimepriceTable->GetFirstPageId());
    }
    ...
    if (head->GetRootId("ticketleft1" , first_page)) {
        ...
        head->InsertRecord("ticketleft1",ticketleftTable1->GetFirstPageId());
    }
    if (head->GetRootId("ticketleft2" , first_page)) {
        ...
        head->InsertRecord("ticketleft2",ticketleftTable2->GetFirstPageId());
    }
    if (head->GetRootId("ticketleft3",first_page)) {
        ...
        head->InsertRecord("ticketleft3",ticketleftTable3->GetFirstPageId());
    }
...
bool sjtu::TrainManager::deletetrain (const sjtu::Trainid &trainid) {
    auto txn=transactionManager.Begin();
        try {
            ...
            auto deletedTrainPage= reinterpret_cast<TablePage*>(recordbpm.FetchPage(deleted_train_rid.GetPageId() , 0));
            ...
            recordbpm.UnpinPage(deleted_train_rid.GetPageId() , false , 0);
            ...
            recordbpm.UnpinPage(deleted_train.loctimepricePos.GetPageId() , false , 0);
