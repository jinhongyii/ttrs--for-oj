...
//        MemoryController ticketleftmc;
    ...
    public:
        ...
        inline train queryTrain (const Trainid &trainid , bool &success , Transaction *txn) {
            ...
            auto trainPage= reinterpret_cast<TablePage*>(recordbpm.FetchPage(trainrid.GetPageId() , 0));
            Tuple thistrain;
            trainPage->GetTuple(trainrid,thistrain,txn);
            recordbpm.UnpinPage(trainrid.GetPageId() , false , 0);
            train result;
        ...
        inline bool queryTrain (const Trainid& trainid,Tuple& tuple,RID& rid,Transaction* txn){
            ...
            auto trainPage= reinterpret_cast<TablePage*>(recordbpm.FetchPage(rid.GetPageId() , 0));
            recordbpm.UnpinPage(rid.GetPageId() , false , 0);
